...
class ICP:
    ...
    def calcTransform(self,A0,B0):
        ...
        B=np.array(B0)
        iter=0
        error=self.calcError(A,B,R,t)
        while(error>self.max_error and iter<self.max_iter):
            A_new,B_new=self.findNearest(A,B) #重新排列A和B
            delta_R,delta_t=self.stepForward(A_new,B_new) #按线性代数方法计算delta_R和delta_t
            t=t+np.dot(R,delta_t)#更新R和t
            R=np.dot(R,delta_R)
            error=self.calcError(A0,B0,R,t) #R和t对应的误差
            print("error=",error)
            # A=np.dot(R,A0)+t #利用本次迭代计算的delta_R和delta_t计算更新A
            A=np.dot(R.T,A-t)
            iter+=1
        return R,t
