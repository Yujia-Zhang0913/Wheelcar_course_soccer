...
if __name__ == '__main__':
	...
	dt=0.1 if IS_SIMPLE_MODE else 0.12 #仿真步长
	i=1 #奇数从右上到左下，偶数从左下到右上
	flag=1 #到达终点的标志
	flag_start=0 #程序启动标志
	...
	while True:
		...
		blue_robot_xy=[np.array([vision.blue_robot[j].x,vision.blue_robot[j].y]) for j in range(1,16) if vision.blue_robot[j].x!=-999999 and vision.blue_robot[j].y!=-999999]
		yellow_robot_xy=[np.array([vision.yellow_robot[j].x,vision.yellow_robot[j].y]) for j in range(0,16) if vision.yellow_robot[j].x!=-999999 and vision.yellow_robot[j].y!=-999999]
		obstacles=np.array(blue_robot_xy+yellow_robot_xy)
		if flag_start==0:
			obstacles_vel=np.zeros(obstacles.shape)
			flag_start=1
		else:
			obstacles_vel=(obstacles-obstacles_prev)/dt #计算障碍物的运动速度
		obstacles_prev=np.array(obstacles)
		my_robot_xy=np.array([myRobot.x,myRobot.y])
		# print(obstacles)
		...
		if flag==1:#若到达终点则进行重规划
			...
			print("Global path plannig finish!")
			...
			if FILTERED_PATH=='TRUE':
				best_path_X=best_path_X_filtered
				best_path_Y=best_path_Y_filtered
		...
		if TRAJECTORY_METHOD=="FEEDBACK":
			...
			# # 想要使用反馈控制，注释下10行
			dwa=DWA()
			dwaconfig=Config(obs_radius=150,predict_time=0.65,predict_time_collision=2,gain_heading=0.6,gain_avoid_collision=0.25,gain_velocity=0.15,w_max=15,dt=dt) \
			if IS_SIMPLE_MODE\
				else Config(obs_radius=190,predict_time=0.9,predict_time_collision=2.5,gain_heading=0.55,gain_avoid_collision=0.3,gain_velocity=0.15,w_max=10,dt=dt)
			...
			#选取跟踪点mid_pos
			index_delta=int(20*dwaconfig.predict_time) if IS_SIMPLE_MODE else int(15*dwaconfig.predict_time)
			if dis_index+index_delta+1>len(best_path_Y):#判断midpos是否到达终点
				dis_index=len(best_path_Y)-index_delta-1
				gamma=0.95 if IS_SIMPLE_MODE else 0.7
				dwaconfig.predict_time=dwaconfig.predict_time*gamma #predict time衰减，防止终点附近大幅减速
			midpos=[best_path_X[dis_index+index_delta],best_path_Y[dis_index+index_delta]]
			#dwa规划v和w
			v,w,position_predict=dwa.plan(robot_info, dwaconfig, midpos, obstacles, obstacles_vel)
			action.sendCommand(vx=v, vy=0, vw=w)
			# 3. draw debug msg
		...
		if frame_length<dt:
			time.sleep(dt-frame_length) #如果本帧所用时间不足dt，则等到dt结束
		else:
			print("WARNING:frame_length=",frame_length,"dt=",dt,"frame_length>dt!")